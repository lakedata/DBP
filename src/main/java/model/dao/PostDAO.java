...
public class PostDAO {
	...
	public int deletePost(int postNum) throws SQLException {
		...
		String sql = "DELETE Post " + "WHERE postNum=?";
	...
	public Post findPost(int postNum) throws SQLException {
		...
		try {
			...
			if (rs.next()) {
				post = new Post(rs.getInt("postNum"), rs.getInt("policyId"), rs.getString("user_Id"),
						rs.getString("title"), rs.getString("writeDate"), rs.getString("content"));
			}
	...
	public List<Post> findPostList() throws SQLException {
		...
		try {
			...
			while (rs.next()) {
				Post po = new Post(rs.getInt("postNum"), rs.getInt("policyId"), rs.getString("user_Id"),
						rs.getString("title"), rs.getString("writeDate"), rs.getString("content"));
				postList.add(po); //
			}
			return postList;
	...
	public List<Post> findMyPostList(String userId) throws SQLException {
		...
		String sql = "SELECT postNum, policyId, user_Id, title, to_char(writeDate, 'YYYY-MM-DD') as writeDate, content "
				+ "FROM Post " + "WHERE user_Id = ? " + "ORDER BY postNum";
